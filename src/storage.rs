        ...
        match self.store.get_token(name) {
            Ok(encrypted_token) => {
                let decrypted_token = self
                    ...
                    .map_err(|_| anyhow::anyhow!("Incorrect master key. Cannot decrypt token."))?;
                Ok(decrypted_token)
            }
    ...
    pub fn populate_tokens_to_child(&self) -> Result<()> {
        ...
        for name in self.store.list_tokens()? {
            if let Ok(encrypted_token) = self.store.get_token(&name) {
                ...
                if let Ok(final_decrypted_token) = decrypted_token {
                    child_env.insert(name.clone(), final_decrypted_token);
                }
            }
        }
